...
class RedisClustr {
  constructor(config) {
    ...
    if (config.slotInterval)
      self._slotInterval = setInterval(
        self.getSlots.bind(self),
        config.slotInterval
      );
  }
  ...
  createClient(port, host) {
    ...
    var createClient = self._config.createClient || _createClient;
    var cli = createClient(port, host, self._config.redisOptions);

    return cli;
  }
}

inherits(RedisClustr, Events);
...
RedisClustr.prototype.getClient = function (port, host, master) {
  ...
  cli.on('error', function (err) {
    if (
      ...
      if (err.code === 'CONNECTION_BROKEN' || err.code === 'NR_CLOSED')
...
RedisClustr.prototype.getRandomConnection = function (exclude, forceSlaves) {
  ...
  var masterOnly = !forceSlaves && self._config.slaves === 'never';
...
RedisClustr.prototype.getSlots = function (cb) {
  ...
  if (q) {
    if (!cb) return;
    if (
      self._config.maxQueueLength &&
      q.length >= self._config.maxQueueLength
    ) {
      var err = new Error('Max slot queue length reached');
      if (self._config.queueShift !== false) {
...
RedisClustr.prototype.doCommand = function (cmd, conf, args) {
  ...
  self.parseArgs(args, function (_, args, cb) {
    ...
    if (!key && !conf.keyless)
      return cb(new Error('no key for command: ' + cmd));

    var r = self.selectClient(key, conf);
    if (!r) return cb(new Error("Couldn't get client"));
    if (!r[cmd])
      return cb(new Error("NodeRedis doesn't know the " + cmd + ' command'));
...
RedisClustr.prototype.subscribeAll = function (exclude) {
  ...
  var cli = (self.subscribeClient = self.createClient(
    con.connection_options.port,
    con.connection_options.host
  ));
